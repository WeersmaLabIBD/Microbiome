...
write.table(all,file="WES_cohort_specific_mbQTL_interactions.txt",row.names = FALSE,quote = FALSE,sep = "\t")

significant_P_interaction=read.table("WES_cohort_specific_mbQTL_interactions.txt",header = T,sep = "\t",stringsAsFactors = F)
...
ibd_allele=read.table("IBD_genotype.txt",header = T,stringsAsFactors = F,check.names = F,sep="\t")
lld_allele=read.table("LLD_genotype.txt",header = T,stringsAsFactors = F,check.names = F,sep="\t")
...
library(ggplot2)
pdf("testhaha.pdf",width=10, height=7)
for(i in 1:nrow(significant_P_interaction)){
  ...
  colnames(table)=c("sample","probe","genotype","group")
  ...
  p=ggplot(table, aes(genotype,probe)) + 
    ...
    geom_dotplot(binaxis = "y", stackdir = "center",dotsize = 2,alpha=0.3,binwidth=0.03,aes(fill=group))+
