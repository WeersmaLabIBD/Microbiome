---
title: "DAG VS HMP2 data comparison"
output:
  html_document:
    css: DAG3.css
...
knitr::opts_chunk$set(echo = FALSE, message=FALSE, warning=FALSE, results = 'hide', fig.width=20, fig.height=8, cache = TRUE)
...
# ================================================================================
metaPhlanGetTaxa <- function(inFileA, Ttresh=0.001,ret="S",FUN=mean) {
  taxaA=read.table(inFileA, sep = "\t", row.names = 1, header = T, check.names = F,blank.lines.skip = T)
  taxaA=as.data.frame(t(taxaA))
  # remove strains
...
# ======================================================================
dagTax <-metaPhlanGetStats('./data/Pilot_metaphlan.txt')
dagTax$cohort <- "DAG3_Pilot"

# ======================================================================
# prepare taxonomy data (HMP2)
# ======================================================================
hmp1508Tax <- metaPhlanGetStats('./data/HMP2_IBD_1508_taxprofile.tsv')
hmp1508Tax$cohort <- "HMP1508"
# 316 samples

hmp1729Tax <- metaPhlanGetStats('./data/HMP2_IBD_1729_taxprofile.tsv')
...
# ======================================================================
lldTax <- metaPhlanGetStats('./data/LLD_taxonomy_metaphlan2_092017.txt')
lldTax$cohort <- "LLD"
...
### DAG3 cohort has `r nrow(dagTax)` samples
### LLD cohort has `r nrow(lldTax)` samples
### HMP1508 (pilot) cohort has `r nrow(hmp1508Tax)` samples
### HMP1729 (current data) cohort has `r nrow(hmp1729Tax)` samples
...
# Species overlaps, mean abundance >= 0.0 %

***

```{r cache = TRUE,results='hide',message=FALSE,echo = FALSE,fig.align="center",fig.height=6,fig.width=8}
# check for overlap
dagTaxS = metaPhlanGetTaxa("./data/Pilot_metaphlan.txt",Ttresh = 0.00)
lldTaxS = metaPhlanGetTaxa("./data/LLD_taxonomy_metaphlan2_092017.txt",Ttresh = 0.00)
hmp1508TaxS = metaPhlanGetTaxa("./data/HMP2_IBD_1508_taxprofile.tsv",Ttresh = 0.00)
hmp1729TaxS = metaPhlanGetTaxa("./data/HMP2_IBD_1729_taxprofile.tsv",Ttresh = 0.00)
...
```{r cache = TRUE,results='hide',message=FALSE,echo = FALSE,fig.align="center",fig.height=6,fig.width=8}
plot.new()
grid.draw(venn.diagram(list(LLD=lldTaxS,DAG3=dagTaxS),filename = NULL,fill = 2:3 , alpha = 0.3))
```

# Species overlaps, mean abundance >= 0.05 %

***

```{r cache = TRUE,results='hide',message=FALSE,echo = FALSE,fig.align="center",fig.height=6,fig.width=8}
# check for overlap
dagTaxS = metaPhlanGetTaxa("./data/Pilot_metaphlan.txt",Ttresh = 0.05)
lldTaxS = metaPhlanGetTaxa("./data/LLD_taxonomy_metaphlan2_092017.txt",Ttresh = 0.05)
hmp1508TaxS = metaPhlanGetTaxa("./data/HMP2_IBD_1508_taxprofile.tsv",Ttresh = 0.05)
hmp1729TaxS = metaPhlanGetTaxa("./data/HMP2_IBD_1729_taxprofile.tsv",Ttresh = 0.05)
hmpall = c(hmp1508TaxS,hmp1729TaxS)
```
```{r cache = TRUE,results='hide',message=FALSE,echo = FALSE,fig.align="center",fig.height=6,fig.width=8}
plot.new()
...
# Species overlaps, mean abundance >= 1 %

***

```{r cache = TRUE,results='hide',message=FALSE,echo = FALSE,fig.align="center"}
# check for overlap
dagTaxS = metaPhlanGetTaxa("./data/Pilot_metaphlan.txt",Ttresh = 1.0)
lldTaxS = metaPhlanGetTaxa("./data/LLD_taxonomy_metaphlan2_092017.txt",Ttresh = 1.0)
hmp1508TaxS = metaPhlanGetTaxa("./data/HMP2_IBD_1508_taxprofile.tsv",Ttresh = 1.0)
hmp1729TaxS = metaPhlanGetTaxa("./data/HMP2_IBD_1729_taxprofile.tsv",Ttresh = 1.0)
hmpall = c(hmp1508TaxS,hmp1729TaxS)
```
```{r cache = TRUE,results='hide',message=FALSE,echo = FALSE,fig.align="center",fig.height=6,fig.width=8}
plot.new()
...
# Species overlaps, median abundance >= 0.0 %

***

```{r cache = TRUE,results='hide',message=FALSE,echo = FALSE,fig.align="center"}
# check for overlap
dagTaxS = metaPhlanGetTaxa("./data/Pilot_metaphlan.txt",Ttresh = 0.00,FUN=median)
lldTaxS = metaPhlanGetTaxa("./data/LLD_taxonomy_metaphlan2_092017.txt",Ttresh = 0.00,FUN=median)
hmp1508TaxS = metaPhlanGetTaxa("./data/HMP2_IBD_1508_taxprofile.tsv",Ttresh = 0.00,FUN=median)
hmp1729TaxS = metaPhlanGetTaxa("./data/HMP2_IBD_1729_taxprofile.tsv",Ttresh = 0.00,FUN=median)
hmpall = c(hmp1508TaxS,hmp1729TaxS)
```
```{r cache = TRUE,results='hide',message=FALSE,echo = FALSE,fig.align="center",fig.height=6,fig.width=8}
plot.new()
...
# Species overlaps, median abundance >= 0.05 %

***

```{r cache = TRUE,results='hide',message=FALSE,echo = FALSE,fig.align="center"}
# check for overlap
dagTaxS = metaPhlanGetTaxa("./data/Pilot_metaphlan.txt",Ttresh = 0.05,FUN=median)
lldTaxS = metaPhlanGetTaxa("./data/LLD_taxonomy_metaphlan2_092017.txt",Ttresh = 0.05,FUN=median)
hmp1508TaxS = metaPhlanGetTaxa("./data/HMP2_IBD_1508_taxprofile.tsv",Ttresh = 0.05,FUN=median)
hmp1729TaxS = metaPhlanGetTaxa("./data/HMP2_IBD_1729_taxprofile.tsv",Ttresh = 0.05,FUN=median)
hmpall = c(hmp1508TaxS,hmp1729TaxS)
```
```{r cache = TRUE,results='hide',message=FALSE,echo = FALSE,fig.align="center",fig.height=6,fig.width=8}
plot.new()
...
# Species overlaps, median abundance >= 1 %

***

```{r cache = TRUE,results='hide',message=FALSE,echo = FALSE,fig.align="center"}
# check for overlap
dagTaxS = metaPhlanGetTaxa("./data/Pilot_metaphlan.txt",Ttresh = 1.0,FUN=median)
lldTaxS = metaPhlanGetTaxa("./data/LLD_taxonomy_metaphlan2_092017.txt",Ttresh = 1.0,FUN=median)
hmp1508TaxS = metaPhlanGetTaxa("./data/HMP2_IBD_1508_taxprofile.tsv",Ttresh = 1.0,FUN=median)
hmp1729TaxS = metaPhlanGetTaxa("./data/HMP2_IBD_1729_taxprofile.tsv",Ttresh = 1.0,FUN=median)
hmpall = c(hmp1508TaxS,hmp1729TaxS)
```
```{r cache = TRUE,results='hide',message=FALSE,echo = FALSE,fig.align="center",fig.height=6,fig.width=8}
plot.new()
...
# Most abundant Species abundances (cohort comparison)

***

```{r cache = TRUE,results='hide',message=FALSE,echo = FALSE,fig.align="center"}
# Compare most abundant taxa between cohorts
Ttresh = 0.01
FUN=median
ret="S"

iF <- './data/HMP2_IBD_1729_taxprofile.tsv'
t1 <- metaPhlanGetTaxa(iF,Ttresh,FUN,ret=ret)
# --
iF <- './data/HMP2_IBD_1508_taxprofile.tsv'
t2 <- metaPhlanGetTaxa(iF,Ttresh,FUN,ret=ret)
# --
iF <- './data/Pilot_metaphlan.txt'
t3 <- metaPhlanGetTaxa(iF,Ttresh,FUN,ret=ret)
# --
iF <- './data/LLD_taxonomy_metaphlan2_092017.txt'
t4 <- metaPhlanGetTaxa(iF,Ttresh,FUN,ret=ret)
# --
sTaxa <- intersect(t1,t2)
sTaxa <- intersect(sTaxa,t3)
sTaxa <- intersect(sTaxa,t4)
print (length(sTaxa))

# - extract -
m1 <- metaPhlanExtractTaxa('./data/HMP2_IBD_1729_taxprofile.tsv',sTaxa)
g1 <- gather(m1,key="tax",value="abun",1:ncol(m1))
g1$cohort <- "HMP2"

m2 <- metaPhlanExtractTaxa('./data/HMP2_IBD_1508_taxprofile.tsv',sTaxa)
g2 <- gather(m2,key="tax",value="abun",1:ncol(m2))
g2$cohort <- "HMP2"

m3 <- metaPhlanExtractTaxa('./data/Pilot_metaphlan.txt',sTaxa)
g3 <- gather(m3,key="tax",value="abun",1:ncol(m3))
g3$cohort <- "DAG3"

m4 <- metaPhlanExtractTaxa('./data/LLD_taxonomy_metaphlan2_092017.txt',sTaxa)
g4 <- gather(m4,key="tax",value="abun",1:ncol(m4))
g4$cohort <- "LLD"

all <- rbind(g1,g2,g3,g4)

for (i in 1:nrow(all)) {
  if (ret=="S") all$tax[i] <- strsplit(all$tax[i],"s__")[[1]][2]
  else if (ret=="G") all$tax[i] <- strsplit(all$tax[i],"g__")[[1]][2]
}


#ggplot(all,aes(x=tax,y=abun,color=cohort)) + geom_boxplot() + ylim(0,25)
allnozeros <- all[all$abun !=0.0,]
ggplot(allnozeros,aes(x=tax,y=abun,color=cohort)) + geom_boxplot() + ylim(0,15) + theme(axis.text.x = element_text(angle = 60, hjust = 1,size=12))  + xlab("Species") + ylab("Abundancy")

``` 

***

# Most abundant Genera abundances (cohort comparison)
...
#ggplot(all,aes(x=tax,y=abun,color=cohort)) + geom_boxplot() + ylim(0,25)
allnozeros <- all[all$abun !=0.0,]
ggplot(allnozeros,aes(x=tax,y=abun,color=cohort)) + geom_boxplot() + ylim(0,17) + theme(axis.text.x = element_text(angle = 60, hjust = 1,size=12)) + xlab("Genera") + ylab("Abundancy")

# clean junk log files
