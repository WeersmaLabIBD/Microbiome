...
NOTE: the latest version of NGS_NDA is 3.4.3, but the Boxy cluster can not load it. We should install it in our {HOME} directory. 在里面添加umcg-weersma.csv
So do some change in generate_template.sh:
1) module load NGS_DNA/3.4.3-beta
2) module load Python/2.7.11-foss-2015b
# 这句命令需要两步操作
# 当加 module load Python/2.7.11-foss-2015b 这句命令时，报错：
...
# 删除 module load Python/2.7.11-foss-2015b 这句命令，重新运行：
...
# 总结： 先用Python 2.7 运行，前半部分不报错，后半部分报错。 再用Python 3运行，前半部分报错，后半部分不报错
...
7.Calculate QC metrics on raw data
sh s03_FastQC_0.sh
