...
testAbundances <- function(dataIn,saveFolder,pathway=T,doSave=T,display="P",onlyShowSig=T, 
...
testOneFeature <- function(dataIn,saveFolder,feature=NA,doSave=T,display="P",onlyShowSig=T,yLab=NA, doViolin=T,title=NA,
                           responseVar="Diagnosis",stTest="wilcox",doPlots = T,nrTests=-1,xLab=NA,retPlot=F,ylim=NA,alpha=0.25)
{
  ...
  pws <- grep(feature,colnames(dataIn))
  ftr=feature
  ymax = 1000000
    ...
    if (doPlots) {
      ...
      if (nRows > 0) {
        for (f in seq(1,length(classes)-1)) {
          ...
          if (fdr < 0.05) {rowCnt <- 1; ypos <- min(ymax,max(dataIn[[i]],na.rm = T))*(1+rowMove*rowCnt-0.15); 
             yposLine <- min(ymax,max(dataIn[[i]],na.rm = T))*(1+rowMove*rowCnt-rowTextAdj)}
          if (fdr < 0.05) {lbl <- paste(lbl,"*"); sigP = "s"; sigN = sigN + 1; doShow = T}
          if (fdr < 0.0005) {lbl <- paste(lbl,"*",sep="");sigP = "s"; sigN = sigN + 1; doShow = T}
          if (doShow) {
      ...
      if (nRows > 1) {
        ...
        if (!onlyShowSig) {doShow = T; rowCnt <- rowCnt + 1;ypos <- min(ymax,max(dataIn[[i]],na.rm = T))*(1+rowMove*rowCnt-0.15); 
                          yposLine <- min(ymax,max(dataIn[[i]],na.rm = T))*(1+rowMove*rowCnt-rowTextAdj)}
        if (fdr < 0.05) {if (onlyShowSig) {rowCnt <- rowCnt + 1}; ypos <- min(ymax,max(dataIn[[i]],na.rm = T))*(1+rowMove*rowCnt-0.15); 
                          yposLine <- min(ymax,max(dataIn[[i]],na.rm = T))*(1+rowMove*rowCnt-rowTextAdj)}
        if (fdr < 0.05) {lbl <- paste(lbl,"*"); sigP = "s"; sigN = sigN + 1; doShow = T}
        if (fdr < 0.0005) {lbl <- paste(lbl,"*",sep=""); sigP = "s"; sigN = sigN + 1; doShow = T}
        if (doShow) {
...
# final adjustment for axis
      ...
      if (doSave) {
        fn <- paste(saveFolder,'_',sigP,sigN,'_',i,'.png',sep='')
        print (fn)
